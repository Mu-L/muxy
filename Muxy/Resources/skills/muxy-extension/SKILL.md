...
- **Embed a live web page with `muxy.browser.init(element, { url?, profile? })`** — for an actual browser (not just an API call). It returns a handle backed by a native `WKWebView` whose origin is the real site, so there is **no CORS**, full JS, subresources, and sessions all work — what `<iframe>` and the webview's own `fetch()` can't do. You ship the chrome (address bar, tabs) and reserve a placeholder element; Muxy renders the viewport on top, tracking its rect on scroll/resize. The handle exposes `loadURL/back/forward/reload/stop`, live `url/title/canGoBack/isLoading/progress`, `find`, `executeJS`, `show/hide`, `on(event, cb)` (`did-navigate`, `title-changed`, `loading-changed`, `progress` — each fires only on a real change — plus `state` and `destroyed`), and `destroy()` (always call it when removing the element). Call `init` multiple times for a tab strip of viewports. Profiles are extension-scoped and persistent — `muxy.browser.profiles.create/list/clear/delete`, and `setCookies(profile, cookies)` to seed a session (you own importing from Chrome/etc.; Muxy stays generic). Needs `browser:embed`; the user is prompted once the first time the extension embeds a browser, then navigation runs freely (it's a real browser — no per-host prompts, no host blocking). Webview-only surface. See [Browser](https://muxy.app/docs/extensions/browser).
